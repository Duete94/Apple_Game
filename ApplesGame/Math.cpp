...
namespace ApplesGame
{
	...
	void SetSpriteSize(sf::Sprite& sprite, float desiredWidth, float desiredHeight)
	{
		...
		sf::Vector2f scale = { desiredWidth / spriteRect.width, desiredHeight / spriteRect.height }; //Делим каждую координату Желаемого размера на каждую Координату размера спрайта
	...
	void SetSpriteRelativeOrigin(sf::Sprite& sprite, float originX, float originY)
	{
		...
		sf::FloatRect spriteRect = sprite.getLocalBounds();
		sprite.setOrigin(originX * spriteRect.width, originY * spriteRect.height);  //Расчет позиции Origin
